...
test_that("mcee: availability not provided -> message; binary checks; dp & outcome checks", {
  ...
  d4 <- dat; d4$Y[d4$id == 1][1] <- d4$Y[d4$id == 1][1] + 1
  expect_error(
    mcee(
  ...
  d5 <- dat; swap <- which(d5$id==2)[2:3]; d5$dp[swap] <- rev(d5$dp[swap])
  expect_error(
    mcee(
...
test_that("mcee: moderator & control formula checks; rand_prob checks; weights", {
  ...
  expect_error(
    mcee(dat, "id","dp","Y","A","M","I", rand_prob=0.5,
  ...
  d <- dat; d$p <- 0.5; d$p[d$I==1][1] <- 0   # invalid at an available row
  expect_error(
...
test_that("mcee_general: config validation & shared messages", {
  ...
  expect_error(
    mcee_general(dat, "id","dp","Y","A","M", availability="I",
                 time_varying_effect_form = ~ 1,
...
test_that("mcee_userfit_nuisance: supplied vectors length/values; availability overrides", {
  ...
  p1_bad <- p1; p1_bad[dat$I==0] <- 0.7
  q1_bad <- q1; q1_bad[dat$I==0] <- 0.2
  expect_warning(
    mcee_userfit_nuisance(dat, "id","dp","Y","A","M","I",
                          time_varying_effect_form=~1,
                          p1=p1_bad, q1=q1_bad,
